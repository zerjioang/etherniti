...
func onNewStatusData(serializer io.Serializer) []byte {
	if serializer == nil {
	...
	raw := ioproto.GetBytesFromSerializer(serializer, wrapper)
...
func onNewIntegrityData(serializer io.Serializer) []byte {
	if serializer == nil {
		serializer = fallbackSerializer
	}
	...
	return ioproto.GetBytesFromSerializer(serializer, wrapper)
}
