...
func LoadIndexConstants() {
	// load constants
	IndexWelcomeJson = `{"name":"eth-wbapi","description":"Etherniti: High Performance Ethereum REST API Proxy","cluster_name":"eth-wbapi","version":"` + constants.Version + `","commit":"` + banner.Commit + `","env":"` + config.EnvironmentName + `","tagline":"dapps everywhere"}`
