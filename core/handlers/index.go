...
func init() {
	...
	// load constants
	IndexWelcomeJson = `{"name":"eth-wbapi","description":"Etherniti:High Performance Ethereum REST API","cluster_name":"eth-wbapi","version":"` + release.Version + `","commit":"` + util.Commit + `","env":"` + config.EnvironmentName + `","tagline":"dapps everywhere"}`
