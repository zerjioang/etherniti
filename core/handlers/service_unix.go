...
func (deployer HttpDeployer) GetLocalHostTLS() (tls.Certificate, error) {
	return localhostCert, certEtr
}

func (deployer HttpDeployer) Run() {
...
func (deployer HttpDeployer) shutdown(httpInstance *echo.Echo, httpsInstance *echo.Echo) {
...
func (deployer HttpDeployer) buildSecureServerConfig(e *echo.Echo) (*http.Server, error) {
...
func (deployer HttpDeployer) buildInsecureServerConfig(e *echo.Echo) (*http.Server, error) {
	//configure custom secure server
	return &http.Server{
		Addr:         config.ListeningAddress,
		ReadTimeout:  3 * time.Second,
...
func (deployer HttpDeployer) newServerInstance() *echo.Echo {
	// build a the server
	e := echo.New()
	// enable debug mode
