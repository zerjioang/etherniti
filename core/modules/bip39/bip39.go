...
var (
	...
	// an invalid size.
	ErrEntropyLengthInvalid = trycatch.New("entropy length must be [128, 256] and a multiple of 32")
	...
	// sanity assertion.
	ErrValidatedSeedLengthMismatch = trycatch.New("seed length does not match validated seed length")
...
func NewEntropy(bitSize int) ([]byte, trycatch.Error) {
	...
	return entropy, trycatch.Ret(rErr)
}
...
// An trycatch is returned if the given mnemonic is invalid.
func EntropyFromMnemonic(mnemonic string) ([]byte, trycatch.Error) {
...
// the given entropy.
// If the provide entropy is invalid, an trycatch error will be returned.
func NewMnemonic(entropy []byte) (string, trycatch.Error) {
	...
	return strings.Join(words, " "), trycatch.Nil()
}
...
func MnemonicToByteArray(mnemonic string, raw ...bool) ([]byte, trycatch.Error) {
	...
	if len(raw) > 0 && raw[0] {
		return rawEntropyBytes, trycatch.Nil()
	}

	return checksummedEntropyBytes, trycatch.Nil()
}

// NewSeedWithErrorChecking creates a hashed seed output given the mnemonic string and a password.
// An trycatch is returned if the mnemonic is not convertible to a byte array.
func NewSeedWithErrorChecking(mnemonic string, password string) ([]byte, trycatch.Error) {
	_, err := MnemonicToByteArray(mnemonic)
	if err.Occur() {
		return nil, err
	}
	return NewSeed(mnemonic, password), trycatch.Nil()
}
...
// mnemonic.
func validateEntropyBitSize(bitSize int) trycatch.Error {
